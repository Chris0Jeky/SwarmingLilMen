...
- **Approach**: Introduced `SwarmSim.Core.Canonical` with `Vec2`, immutable `Boid`, `RuleContext`, `IRule`, `ISpatialIndex`, `CanonicalWorld`, and a naive spatial index. `CanonicalWorld` filters neighbors by radius/FOV, applies separation/alignment/cohesion rules that compute desired headings and clamp steering to `MaxForce`, and re-normalizes velocities to the configured `TargetSpeed`. `CanonicalBoidsTests` now exercise Vec2 math, perception guards, deterministic stepping, and each steering rule, providing the TDD coverage described in `NewImplementation.md`.
...
- **How to run**: Run the canonical harness via `dotnet test --filter CanonicalBoidsTests` to ensure the new world steps deterministically. To see it in action, instantiate `CanonicalWorld` with a `NaiveSpatialIndex`, attach the new rules (the defaults are already registered), and step it inside your own loop or renderer—once that wiring is in place, `dotnet run --project SwarmSim.Render` will show the canonical behavior instead of the legacy pipeline.
